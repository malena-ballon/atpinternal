...
import type { ExamRow, StudentRow, ScoreRow } from '@/types'
...
export default function BulkScoreModal({ exam, classId, classStudents, classPassingPct, onClose, onImported }: Props) {
  ...
  const studentEmailMap = new Map(
    classStudents.map(s => [s.email?.toLowerCase() ?? '', s])
  )
  ...
  function handlePreview() {
    ...
    const scores = parseColumn(scoresText)

    if (emails.length === 0) { setError('Email column is empty.'); return }
    if (scores.length === 0) { setError('Score column is empty.'); return }
    if (emails.length !== scores.length) {
      setError(`Row count mismatch — Emails: ${emails.length}, Scores: ${scores.length}. Each column must have the same number of rows.`)
      return
    }
    ...
    // Parse scores
    const unparseable = scores.map((s, i) => ({ s, i })).filter(({ s }) => !parseScore(s))
    if (unparseable.length > 0) {
      setError(`Could not parse score on row(s): ${unparseable.map(x => x.i + 1).join(', ')}. Expected format: "25 / 50"`)
      return
    }
    ...
    if (totals.size > 1) {
      setTotalWarning(`Different denominators detected: ${[...totals].join(', ')}. The most common value will be used as Total Items.`)
    }
  ...
  async function handleImport() {
    ...
    const validParsed = preview.filter(r => r.parsed).map(r => r.parsed!)
    ...
    for (const row of toImport) {
      ...
      const { data: score, error: err } = await supabase
        .from('scores')
        .upsert(
          { exam_id: exam.id, student_id: studentId, raw_score: row.parsed.score, total_items: row.parsed.total },
    ...
    // Update exam's total_items with detected value
      ...
      {!preview ? (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            {([
              { label: 'Student Name', required: false, value: namesText, set: setNamesText, placeholder: 'Juan Dela Cruz\nMaria Santos\n(optional)' },
              { label: 'Email', required: true, value: emailsText, set: setEmailsText, placeholder: 'juan@email.com\nmaria@email.com\n...' },
              { label: 'Score', required: true, value: scoresText, set: setScoresText, placeholder: '48 / 50\n25 / 50\n30 / 50\n...' },
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ backgroundColor: 'var(--color-bg)', borderBottom: '1px solid var(--color-border)' }}>
                  {['#', 'Name', 'Email', 'Score', '%', 'Result', 'Status'].map(h => (
              ...
              <tbody>
                {preview.map((row, i) => {
                  const passes = row.parsed ? row.parsed.pct >= effectivePct : null
                  return (
                    <tr key={i} style={{
                      ...
                      <td className="px-3 py-2.5 font-mono text-xs" style={{ color: 'var(--color-text-secondary)' }}>
                        {row.parsed ? `${row.parsed.score} / ${row.parsed.total}` : (
                          <span style={{ color: 'var(--color-danger)' }}>Invalid: {row.rawScore}</span>
                        )}
                      </td>
                      <td className="px-3 py-2.5 font-mono text-xs font-medium" style={{ color: 'var(--color-text-primary)' }}>
                        {row.parsed ? `${row.parsed.pct.toFixed(1)}%` : '—'}
                      </td>
